...
class PostsController extends AppController {
    ...
    //  サーバー変えたりしたら ~/posts/register_instagram を１度叩くが、二度と実行する必要はない
    public function register_instagram(){
        ...
        // test code
        $callback_url = 'http://k0hei.science/planbox_instatest/posts/register_instagram' ;        // コールバックURL
        ...
        if(strpos($object, "user")!==false){
            ...
            foreach ($photos_data->data as $item) {
                ...
                if(array_search("planbox", $tags)!==false || array_search("Planbox", $tags)!==false){
                    ...
                    //
                    $post_photo = array('Photo' => array('date_id' => $date_id, 'photo' => $photo, 'location' => $location_name, 'created' => $date, 'modified' => $date ));
                    // 登録するフィールド
                    $fields_photo = array('date_id', 'photo', 'location', 'created', 'modified');

                    $this->Photo->save($post_photo, false, $fields_photo);
